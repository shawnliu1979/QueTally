...
3. 分别右键 `cloudfunctions/game` 和 `cloudfunctions/cleanup`，选择“上传并部署：云端安装依赖”。
4. 在云开发控制台创建集合 `profiles`、`games` 与 `transfers`。云函数使用服务端权限，三个集合不需要向客户端开放读写权限。
...
## 带完成事项
- 用户头像缓存在CloudBase，需要付费版才允许开放其他人访问，所以现在只能看到自己的头像（无耻的企鹅）